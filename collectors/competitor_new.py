...
def _parse_from_html(html: str, brand_name: str) -> list[dict]:
    results = []
    # __NEXT_DATA__ 우선
    ...
    for sel in [
        ...
        "li[class*='item']", ".product-item", "[class*='ProductItem']",
    ]:
        els = soup.select(sel)
        if len(els) >= 2:
            for el in els[:COMPETITOR_NEW_LIMIT]:
                name_el = el.select_one(
                    "[class*='goods-name'],[class*='item-name'],[class*='name'],[class*='title'],strong"
                )
...
def collect_brand_new_products(brand: dict, log_callback=None) -> list[dict]:
    ...
    for url in urls:
        try:
            ...
            results = _parse_from_html(resp.text, brand_name)
            if results:
                return results
        except Exception as e:
            if log_callback:
                log_callback(f"[경쟁사신상품] {brand_name} 오류({url}): {e}")
